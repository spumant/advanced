...
def Binary_Cross_Combination(colNames, features, OneHot=True):
    ...
    if OneHot == True:
        ...
        features_new = pd.DataFrame(
            enc.fit_transform(features_new).toarray(), columns=colNames_new
        )
...
def Binary_Group_Statistics(
    ...
    if col_num != None:
        ...
        # 创建衍生特征df
        features_num_new = (
            features[col_num + [keyCol]].groupby(keyCol).agg(aggs_num).reset_index()
        )
        ...
        if col_cat != None:
            ...
            for key in aggs_cat:
                cols_cat.extend(
                    [key + '_' + keyCol + '_' + stat for stat in aggs_cat[key]]
                )

            # 创建衍生特征df
            features_cat_new = (
                features[col_cat + [keyCol]].groupby(keyCol).agg(aggs_cat).reset_index()
            )
            features_cat_new.columns = cols_cat

            # 合并连续变量衍生结果与离散变量衍生结果
            df_temp = pd.merge(
                features_num_new, features_cat_new, how='left', on=keyCol
            )
        ...
        # 当只有连续变量时
            ...
            features_new = pd.merge(
                features[keyCol], features_num_new, how='left', on=keyCol
            )
            features_new.loc[:, ~features_new.columns.duplicated()]
    ...
    # 当没有输入连续变量时
        ...
        if col_cat != None:
            ...
            features_new = pd.merge(
                features[keyCol], features_cat_new, how='left', on=keyCol
            )
            features_new.loc[:, ~features_new.columns.duplicated()]
    ...
    if quant:
        ...
        features_temp = (
            features[colNames + [keyCol]].groupby(keyCol).agg(aggs).reset_index()
        )
...
def Binary_PolynomialFeatures(colNames, degree, features):
    ...
    for col_index, col_name in enumerate(colNames):
        for col_sub_index in range(col_index + 1, len(colNames)):
            ...
            array_new_temp = PolynomialFeatures(
                degree=degree, include_bias=False
            ).fit_transform(features[col_temp])
...
def Group_Statistics_Extension(colNames, keyCol, features):
    ...
    for col_temp in colNames:
        ...
        features_new[col_temp + '_dive1_' + col] = features_new[keyCol] / (
            features_new[col] + 1e-5
        )
        colNames_new.append(col_temp + '_dive1_' + col)
        col = col_temp + '_' + keyCol + '_' + 'median'
        features_new[col_temp + '_dive2_' + col] = features_new[keyCol] / (
            features_new[col] + 1e-5
        )
    ...
    for col_temp in colNames:
        col = col_temp + '_' + keyCol + '_' + 'mean'
        features_new[col_temp + '_minus1_' + col] = (
            features_new[keyCol] - features_new[col]
        )
        colNames_new.append(col_temp + '_minus1_' + col)
        features_new[col_temp + '_minus2_' + col] = (
            features_new[keyCol] - features_new[col]
        )
    ...
    for col_temp in colNames:
        ...
        features_new[col_temp + '_norm_' + keyCol] = (
            features_new[keyCol] - features_new[col_mean]
    ...
    for col_temp in colNames:
        ...
        features_new[col_temp + '_gap_' + keyCol] = (
            features_new[col_q2] - features_new[col_q1]
        )
    ...
    for col_temp in colNames:
        ...
        features_new[col_temp + '_mag1_' + keyCol] = (
            features_new[col_median] - features_new[col_mean]
        )
        colNames_new.append(col_temp + '_mag1_' + keyCol)
        features_new[col_temp + '_mag2_' + keyCol] = features_new[col_median] / (
            features_new[col_mean] + 1e-5
        )
    ...
    for col_temp in colNames:
        col_mean = col_temp + '_' + keyCol + '_' + 'mean'
        col_var = col_temp + '_' + keyCol + '_' + 'var'
        features_new[col_temp + '_cv_' + keyCol] = np.sqrt(features_new[col_var]) / (
            features_new[col_mean] + 1e-5
        )
...
def Multi_Group_Statistics(
    ...
    if col_num != None:
        ...
        # 生成主键
        features_key2, col2 = Multi_Cross_Combination(
            keyCol, features_num_new, OneHot=False
        )
        ...
        if col_cat != None:
            ...
            for key in aggs_cat:
                cols_cat.extend(
                    [key + '_' + col1 + '_' + stat for stat in aggs_cat[key]]
                )
            ...
            # 生成主键
            features_key3, col3 = Multi_Cross_Combination(
                keyCol, features_cat_new, OneHot=False
            )
            ...
            # merge连续变量衍生结果与原始数据，然后删除重复列
            features_new = pd.merge(
                features_key1, features_num_new, how='left', on=col1
            )
        ...
        if col_cat != None:
            ...
            # 生成主键
            features_key4, col4 = Multi_Cross_Combination(
                keyCol, features_cat_new, OneHot=False
            )
            ...
            features_new = pd.merge(
                features_key1, features_cat_new, how='left', on=col1
            )
            features_new = features_new.loc[:, ~features_new.columns.duplicated()]
    ...
    if quant:
        ...
        # 生成主键
        features_key4, col4 = Multi_Cross_Combination(
            keyCol, features_temp, OneHot=False
        )
...
def Multi_PolynomialFeatures(colNames, degree, features):
    ...
    while deg <= degree:
        ...
        colNames_new_l.extend(
            '&'.join(colNames) + '_' + ''.join([str(i) for i in i]) for i in a3
        )
        deg += 1
...
def timeSeriesCreation(timeSeries, timeStamp=None, precision_high=False):
    ...
    features_new[colNames + '_weekend'] = (
        features_new[colNames + '_dayofweek'] > 5
    ).astype(int)

    if precision_high != False:
        features_new['hour_section'] = (features_new[colNames + '_hour'] // 6).astype(
            int
        )
    ...
    for timeStamp, timeStampName in zip(timeStamp_l, timeStamp_name_l):
        ...
        features_new['time_diff_months' + '_' + timeStampName] = np.round(
            features_new['time_diff_days' + '_' + timeStampName] / 30
        ).astype('int')
        if precision_high != False:
            features_new[
                'time_diff_seconds' + '_' + timeStampName
            ] = time_diff.dt.seconds
            features_new['time_diff_h' + '_' + timeStampName] = time_diff.values.astype(
                'timedelta64[h]'
            ).astype('int')
            features_new['time_diff_s' + '_' + timeStampName] = time_diff.values.astype(
                'timedelta64[s]'
            ).astype('int')
...
# NLP特征衍生函数
def NLP_Group_Statistics(features, col_cat, keyCol=None, tfidf=True, countVec=True):
    """
    NLP特征衍生函数
        ...
        if tfidf == True:
            ...
            if countVec == True:
                ...
                features_new = pd.concat(
                    [features_new_cntv.iloc[:, :n], features_new_tfv], axis=1
                )
    ...
    if keyCol is None:
        ...
        if n == 1:
            ...
            # 将分组统计结果拼接回原矩阵
            features_new = pd.merge(
                features[keyCol[0]], features_new, how='left', on=keyCol[0]
            )
        ...
        # 如果是多特征交叉分组
            ...
            features_new, colNames_new = NLP_Stat()
            # 在原数据集中生成合并主键
            features_key1, col1 = Multi_Cross_Combination(
                keyCol, features, OneHot=False
            )
            # 在衍生特征数据集中创建合并主键
            features_key2, col2 = Multi_Cross_Combination(
                keyCol, features_new, OneHot=False
            )
...
def Features_Padding(
    features_train_new, features_test_new, colNames_train_new, colNames_test_new
):
    """
    特征零值填补函数
...
def test_features(keyCol, X_train, X_test, features_train_new, multi=False):
    """
    测试集特征填补函数
    ...
    if multi == False:
        ...
        train_key, train_col = Multi_Cross_Combination(
            colNames=keyCol, features=X_train, OneHot=False
        )
        test_key, test_col = Multi_Cross_Combination(
            colNames=keyCol, features=X_test, OneHot=False
        )
...
def Cross_Combination(colNames, X_train, X_test, multi=False, OneHot=True):
    """
    交叉组合特征衍生函数
    ...
    if multi == False:
        features_train_new, colNames_train_new = Binary_Cross_Combination(
            colNames=colNames, features=X_train, OneHot=OneHot
        )
        features_test_new, colNames_test_new = Binary_Cross_Combination(
            colNames=colNames, features=X_test, OneHot=OneHot
        )
    else:
        features_train_new, colNames_train_new = Multi_Cross_Combination(
            colNames=colNames, features=X_train, OneHot=OneHot
        )
        features_test_new, colNames_test_new = Multi_Cross_Combination(
            colNames=colNames, features=X_test, OneHot=OneHot
        )
...
def Polynomial_Features(colNames, degree, X_train, X_test, multi=False):
    """
    多项式特征衍生函数
    ...
    """
    if multi == False:
        features_train_new, colNames_train_new = Binary_PolynomialFeatures(
            colNames=colNames, degree=degree, features=X_train
        )
        features_test_new, colNames_test_new = Binary_PolynomialFeatures(
            colNames=colNames, degree=degree, features=X_test
        )
    else:
        features_train_new, colNames_train_new = Multi_PolynomialFeatures(
            colNames=colNames, degree=degree, features=X_train
        )
        features_test_new, colNames_test_new = Multi_PolynomialFeatures(
            colNames=colNames, degree=degree, features=X_test
        )
...
def Group_Statistics(
    ...
    if multi == False:
        # 进行双变量的交叉衍生
        features_train_new, colNames_train_new = Binary_Group_Statistics(
            keyCol=keyCol,
            features=X_train,
            col_num=col_num,
            col_cat=col_cat,
        ...
        if extension == True:
            ...
            features_train_new_ex, colNames_train_new_ex = Group_Statistics_Extension(
                colNames=colNames, keyCol=keyCol, features=X_train
            )

            features_train_new = pd.concat(
                [features_train_new, features_train_new_ex], axis=1
            )
            colNames_train_new.extend(colNames_train_new_ex)

    else:
        # 进行多变量的交叉衍生
        features_train_new, colNames_train_new = Multi_Group_Statistics(
            keyCol=keyCol,
            features=X_train,
            col_num=col_num,
            col_cat=col_cat,
    ...
    (
        ...
        colNames_train_new,
        colNames_test_new,
    ) = test_features(keyCol, X_train, X_test, features_train_new, multi=multi)
    # 如果特征不一致，则进行0值填补
    # 对于分组统计特征来说一般不会出现该情况
    if colNames_train_new != colNames_test_new:
        (
...
def Target_Encode(
    ...
    for trn_idx, val_idx in folds.split(train):
        ...
        (
            trn_temp_new,
            val_temp_new,
            colNames_trn_temp_new,
            colNames_val_temp_new,
        ) = Group_Statistics(
            keyCol,
            X_train=trn_temp,
            X_test=val_temp,
...
def timeSeries_Creation(
    timeSeries_train, timeSeries_test, timeStamp=None, precision_high=False
):
    """
    时序字段的特征衍生
    ...
    :return features_new, colNames_new：返回创建的新特征矩阵和特征名称
    """
    features_train_new, colNames_train_new = (
        features_new,
        colNames_new,
    ) = timeSeriesCreation(
        timeSeries=timeSeries_train, timeStamp=timeStamp, precision_high=precision_high
    )

    features_test_new, colNames_test_new = (
        features_new,
        colNames_new,
    ) = timeSeriesCreation(
        timeSeries=timeSeries_test, timeStamp=timeStamp, precision_high=precision_high
    )
...
def NLP_Group_Stat(X_train, X_test, col_cat, keyCol=None, tfidf=True, countVec=True):
    """
    NLP特征衍生函数
    ...
    # 在训练集上进行NLP特征衍生
    features_train_new, colNames_train_new = NLP_Group_Statistics(
        features=X_train, col_cat=col_cat, keyCol=keyCol, tfidf=tfidf, countVec=countVec
    )
    # 如果不分组，则测试集单独计算NLP特征
    if keyCol == None:
        features_test_new, colNames_test_new = NLP_Group_Statistics(
            features=X_test,
            col_cat=col_cat,
            keyCol=keyCol,
            tfidf=tfidf,
